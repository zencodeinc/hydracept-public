...
Published plugin catalog: [github.com/zencodeinc/hydracept-agent-plugins](https://github.com/zencodeinc/hydracept-agent-plugins).
...
Canonical listing copy is sourced from `public/agents/source/marketplace/copy.yaml`:
...
All marketplace artifacts share `distributionVersion` from `public/agents/source/identity.yaml`. Do not derive it from the Python SDK version. The same identity file carries `minimumHydraceptVersion`, which is the minimum compatible CLI contract for generated integration instructions. MCP Registry publications are immutable per version; bump `distributionVersion` before republishing metadata.
...
1. Run `python -m hydracept init` in your project. The supported minimum CLI contract is `minimumHydraceptVersion` from `public/agents/source/identity.yaml`. Init binds project MCP to stdio. Reload MCP once. Do not copy the workspace key into host plugin settings.
...
- [x] Publish the tested agent-plugin repository via `python scripts/sync_agent_plugins.py`
- [x] Submit `zencodeinc/hydracept-agent-plugins` to the Cursor Marketplace — awaiting review; no listing URL has been returned yet
- [ ] Run `mcp-publisher login http --domain hydracept.com` + `mcp-publisher publish` from `dist/mcp-registry/` (private key stays local)
- [ ] Record listing URLs in `architecture/hydracept-launch-gate.yaml` → `marketplace_listings` when a marketplace returns one

**Later:** submit Hydracept to Anthropic's official third-party plugin directory after the self-hosted Claude marketplace is proven.

OpenAI/Codex, ClawHub, VS Code, JetBrains, and Unity Asset Store listings remain separate publication decisions.
