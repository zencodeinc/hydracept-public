...
public sealed class HydraceptClient : IHydraceptClient
{
    ...
    public Task<JsonElement> CreatePinnedInferenceBulkAsync(
        object body,
        CancellationToken cancellationToken = default) =>
        PostJsonAsync("v1/inference/pinned/bulk", body, headers: null, cancellationToken);

    public Task<JsonElement> GetPinnedBulkAsync(
        string bulkId,
        CancellationToken cancellationToken = default) =>
        GetJsonAsync($"v1/inference/pinned/bulk/{Uri.EscapeDataString(bulkId)}", cancellationToken);
